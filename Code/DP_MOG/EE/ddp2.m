function [results,logli] = ddp2(given_data, opts)
%
...
if opts.sis > 0
  [q_of_z,logli] = sequential_importance_sampling(data, hp_prior, opts);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [q_of_z,logli] = sequential_importance_sampling(data, hp_prior, opts);
disp('start SIS.')
logli=zeros(1,opts.sis);
...
ww= mk_q_of_z(data, hp_posterior, hp_prior, opts);
tt=mk_hp_posterior(data, ww, hp_prior, opts);
[free_energy, qq, fafa] = split_merge(data, tt, hp_prior, opts);
logli(r)=mk_log_likelihood(fafa, qq, hp_prior, opts);
...
while 1
  ...
  hp_posterior = mk_hp_posterior(data, q_of_z, hp_prior, opts);
logli=[logli,mk_log_likelihood(data, hp_posterior, hp_prior, opts)];
end
% disp_status(free_energy, hp_posterior, opts);
disp(['### updating posterior ... done.'])
dlmwrite('logli', logli);
