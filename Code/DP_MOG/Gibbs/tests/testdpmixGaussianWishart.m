...
eta0=1*D+1;
alpha=100;
covariance = cov(datass');
m0 = mean(datass, 2);
max_eig = max(eig(covariance));
%B0=eta_0 * max_eig * eye(D) *xi0;
B0=covariance;
%{%}
qq0      = GaussianWishart(5, numdim, xi0, eta0, m0, B0);
...
for iter=1:numiter
  ...
  if docrp == 1
    ...
    if doplot == 1
      ...
      ss = dp.datass;
      plot(ss(1,:),ss(2,:),'.','markersize',1,'linewidth',1);
      [mu sigma] = map(dp.classqq,dp.classnd);
      for ii=1:dp.numclass
        plotellipse(mu(:,ii),sigma(:,:,ii),'r','linewidth',1);
      end
      axis equal; 
