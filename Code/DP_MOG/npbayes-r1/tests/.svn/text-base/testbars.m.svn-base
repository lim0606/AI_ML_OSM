...
imsize       = 5;
noiselevel   = .01;
numbarpermix = [0 ones(1,4)]/5;
numgroup     = 200;
numdata      = 50;
datass       = genbars(imsize,noiselevel,numbarpermix,numgroup,numdata,ones(1,10));
load data_dm_bar.mat
numdim       = imsize^2;
numclass     = imsize*2+2; 
...
%}
  if dohdpcrf > 0
    ...
    Fss=[Fss,hdp2like(hdpcrf,mode,prior)];
    drawnow;
  end
