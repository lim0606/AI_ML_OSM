function [results] = vdpgm_dp(given_data, opts)
%assign every topic a prior
...
if opts.use_kd_tree
  ...
  for c=1:K
    v0 = means(:,c) - hp_prior.m0(:,c);
    q_of_z_c = reshape(q_of_z(:,c), 1, 1, N);
    S = sum(repmat(q_of_z_c,[D,D,1]).*t1, 3) - Nc(c)*means(:,c)*means(:,c)';
    hp_posterior.B{c} = hp_prior.B0{c} ...
        + S ...
        + Nc(c)*hp_prior.xi0(c)*v0*v0'/(hp_posterior.xi(c)); % D*D
  end
else
...
hp_posterior.m = (sum_x + bsxfun(@times,hp_prior.xi0,hp_prior.m0)) ...
...
for c=1:K
  ...
  d = hp_posterior.m(:,c)-hp_prior.m0(:,c); % D*1
  term_eta(1,c) = sum(sum(hp_posterior.inv_B(:,:,c).*(hp_prior.xi0(c)*d*d'),1),2);
  term_eta(2,c) = sum(sum(hp_posterior.inv_B(:,:,c).*hp_prior.B0{c},1),2) - D;
end
E_log_q_p_mean = ...
    + 0.5*D*(hp_prior.xi0(c)./hp_posterior.xi ...
             - log(hp_prior.xi0(c)./hp_posterior.xi) ...
             - 1) ...
    + 0.5*(hp_posterior.eta).* term_eta(1,:);     

psi_sum = sum(psi( (repmat(hp_posterior.eta+1,D,1) - repmat([1:D]',1,K))*0.5 ), 1); % 1*K
E_log_q_p_cov = zeros(1,K);
for c=1:K
E_log_q_p_cov(c) = ...
    0.5*hp_prior.eta0(c)*(log_det_B(c)-detln(hp_prior.B0{c})) ...
    + 0.5*hp_posterior.Nc(c).*psi_sum(c) ...
    + 0.5*(hp_posterior.eta(c)).* term_eta(2,c) ...
    + gamma_multivariate_ln(hp_prior.eta0(c)*0.5,D) ...
...
if isfield(opts, 'xi0')
  ...
  hp_prior.xi0 = 0.01*ones(1,opts.initial_K);
end

[D,N] = size(data.given_data);
if isfield(opts, 'eta0')
  hp_prior.eta0 = opts.eta0;
else
  hp_prior.eta0 = 1*ones(1,opts.initial_K)*D+2;
end
...
if isfield(opts, 'm0')
  ...
  hp_prior.m0=repmat(mean(data.given_data, 2),1,opts.initial_K);
end
...
if isfield(opts, 'B0')
	...
	if D > 16
	  ...
	  max_eig = max(eig(covariance));
       ...
       hp_prior.B0=cell(1,opts.initial_K);
       for i=1:opts.initial_K
          hp_prior.B0{i} = hp_prior.eta0(i) * max_eig * eye(D) * hp_prior.xi0(i);
       end 
