function [results] = vdpgm_lik(given_data, opts)
start_time = clock;
if nargin == 1
  opts = struct();
end
...
if opts.do_greedy
  [free_energy, hp_posterior, data,opts] = greedy(data, hp_posterior, hp_prior, opts);
else
  [free_energy, hp_posterior, data,opts] = split_merge(data, hp_posterior, hp_prior, opts);
end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [free_energy, hp_posterior, data,opts] = greedy(data, hp_posterior, hp_prior, opts);
...
while 1
  ...
  [new_free_energy, new_hp_posterior, new_data, c,opts] = find_best_splitting(data, ...
                                                    hp_posterior, ...
                                                    hp_prior, opts);
  ...
  [new_free_energy, new_hp_posterior, new_data,dummy,opts] = update_posterior2(new_data, ...
                                                    new_hp_posterior, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [free_energy, hp_posterior, data, c,opts] = find_best_splitting(data, ...
                                                  hp_posterior, ...
                                                  hp_prior, opts);
c_max = 10;
K = size(hp_posterior.m, 2);
...
for c = candidates(1:min(c_max, length(candidates)))
  ...
  [sub_f, sub_hp_posteior, dummy, sub_q_of_z,opts] = update_posterior2(sub_data, ...
                                                    sub_hp_posteior, ...
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [free_energy, hp_posterior, data,opts] = split_merge(data, hp_posterior, hp_prior, opts);
%
% split: 
...
%
c_max = 3;
[free_energy, hp_posterior,dummy,dummy2,opts] = update_posterior2(data, hp_posterior, hp_prior, opts, 0, opts.ite);
while 1
  K = size(hp_posterior.m, 2);
  new_free_energy = inf;
  %%% split
...
%}
  ...
  for i=1:min(c_max, size(candidates_for_merging,2))
    ...
    [new_free_energy, new_hp_posterior,dummy,dummy2,opts] = update_posterior2(data, ...
                                                      new_hp_posterior, ...
                                                      hp_prior, opts);    
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [free_energy, hp_posterior, data, q_of_z,opts] = update_posterior2(data, hp_posterior, hp_prior, opts, upkdtree, ite, do_sort);
% update q_of_z: N*K
...
while 1
  ...
  hp_posterior = mk_hp_posterior(data, q_of_z, hp_prior, opts);
  if isfield(data, 'given_data')
  opts.log_likelihood(1+length(opts.log_likelihood)) = mk_log_likelihood(data, hp_posterior, hp_prior, opts);
  end
end
% disp_status(free_energy, hp_posterior, opts);
...
%%disp(['F=' num2str(free_energy)';    Nc=[' num2str(Nc, ' %0.5g ') '];'])
...
%b0r = bkm_b0r(data.given_data,covariance)
hp_prior.B0 = hp_prior.eta0 * max_eig * eye(D) * hp_prior.xi0;
%hp_prior.B0=b0r*D*covariance;
